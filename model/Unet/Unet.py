...
from model.attention.embeding import timestep
from model.common.conv3x3 import Conv3x3
from model.common.resnet import ResAttentionModule


class UNet(Model):
    def __init__(self, ch_input: int, image_size: int, device: torch.device) -> None:
        super(UNet, self).__init__(ch_input, image_size, device)
        ch = 64
        layers = 2
        ts_embed_dims = 1000
        ts_proj_dims = 1000
        ...
        self.first_conv = Conv3x3(self.ch_input, ch, True)      # 3 -> 64
